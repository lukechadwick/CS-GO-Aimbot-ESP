...
namespace AfterBurner
{
    public class D2DRen
    {
        ...
        private List<lBuffer> LayoutContainer = new List<lBuffer>(32);
        ...
        private static System.Drawing.Color GDITransparent = System.Drawing.Color.Black;
        ...
        public D2DRen(IntPtr hwnd, bool limitFPS)
        {
            factory = new Factory();
        ...
        public void BorderedRectangle(int x, int y, int width, int height, float stroke, float borderStroke, int brush, int borderBrush, int interiorBrush)
        {
        ...
        public void DrawText(string text, int font, int brush, int x, int y, bool bufferText = true)
        {
            if (bufferText)
            {
                ...
                if (bufferPos == -1)
                {
         //           this.LayoutContainer.Add(new lBuffer(text, new TextLayout(this.fontFactory, text, this.FontContainer[font], float.MaxValue, float.MaxValue)));
